...
class PlayVideo extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch(`http://ziptubeserverv2-env.eba-zfjkmcip.us-east-1.elasticbeanstalk.com/video/${videoId}`)
            .then(res => res.json())
            .then(
                (result) => {
                    this.setState({
        ...
        fetch(`http://ziptubeserverv2-env.eba-zfjkmcip.us-east-1.elasticbeanstalk.com/video/incrementViewCount/${videoId}`,
            {method: "PATCH"})
    ...
    changeLikes = (plusOrMinus) => {
        const { videoId } = this.props.location.state;
        fetch(`http://ziptubeserverv2-env.eba-zfjkmcip.us-east-1.elasticbeanstalk.com/video/like/${plusOrMinus}/${videoId}`,
            {method: "PATCH"})
            .then( response => response.json() )    // parse body test as JSON
            .then( result => {
                this.setState( {
    ...
    changeDislikes = (plusOrMinus) => {
        const { videoId } = this.props.location.state;
        fetch(`http://ziptubeserverv2-env.eba-zfjkmcip.us-east-1.elasticbeanstalk.com/video/dislike/${plusOrMinus}/${videoId}`,
            {method: "PATCH"})
            .then( response => response.json() )    // parse body test as JSON
            .then( result => {
                this.setState( {
        ...
        if (error) {
            ...
            return (
                <div className="playVideo-container">
                    ...
                    {/* VIDEO */}
                    <video className="playVideo-video" src={`http://ziptubeserverv2-env.eba-zfjkmcip.us-east-1.elasticbeanstalk.com/file/download/${this.state.video.videoId}`} type="video/mp4" controls muted></video>
                    <br />
